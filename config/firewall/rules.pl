...
sub ipblocklist () {
    ...
    if($blocklistsettings{'ENABLE'} eq "on" && $blocklistsettings{'XDP_ACCEL'} eq "on") {
        ...
        # Only flush/delete if chains exist
        if(&firewall_chain_exists("BLOCKLISTIN") == 0) {
            run("$IPTABLES -F BLOCKLISTIN");
            # Try to delete but suppress error if it fails
            system("$IPTABLES -X BLOCKLISTIN 2>/dev/null");
        }
        if(&firewall_chain_exists("BLOCKLISTOUT") == 0) {
            run("$IPTABLES -F BLOCKLISTOUT");
            # Try to delete but suppress error if it fails
            system("$IPTABLES -X BLOCKLISTOUT 2>/dev/null");
        }

        # Flush and delete blocklist-specific chains
        foreach my $blocklist (@blocklists) {
            if(&firewall_chain_exists("${blocklist}_DROP") == 0) {
                run("$IPTABLES -F ${blocklist}_DROP");
                # Try to delete but suppress error if it fails
                system("$IPTABLES -X ${blocklist}_DROP 2>/dev/null");
            }
    ...
    # Check if the blocklist feature is enabled FIRST
    if($blocklistsettings{'ENABLE'} eq "on") {
        # Feature is ENABLED - set up chains
        
        # Flush the ipblocklist chains (only if they exist)
        if(&firewall_chain_exists("BLOCKLISTIN") == 0) {
            run("$IPTABLES -F BLOCKLISTIN");
        } else {
            # Create the chain if it doesn't exist
            run("$IPTABLES -N BLOCKLISTIN");
        }
        ...
        if(&firewall_chain_exists("BLOCKLISTOUT") == 0) {
            ...
            # Create the chain if it doesn't exist
            run("$IPTABLES -N BLOCKLISTOUT");
        }
        
        # Loop through private networks to create exception rules
        ...
        foreach my $blocklist (@blocklists) {
            if(($blocklistsettings{$blocklist}) && ($blocklistsettings{$blocklist} eq "on")) {
                # Blocklist is enabled - set it up
                &ipset_restore($blocklist);
                
                # Create or flush drop chain
                if(&firewall_chain_exists("${blocklist}_DROP") != 0) {
                ...
                # Blocklist is disabled - clean up if exists
                if(&firewall_chain_exists("${blocklist}_DROP") == 0) {
                    run("$IPTABLES -F ${blocklist}_DROP");
                    # Try to delete but suppress error if it fails
                    system("$IPTABLES -X ${blocklist}_DROP 2>/dev/null");
        ...
        # Feature is DISABLED - just flush the chains if they exist
        # Don't try to delete them as they might be referenced elsewhere
        
        if(&firewall_chain_exists("BLOCKLISTIN") == 0) {
            run("$IPTABLES -F BLOCKLISTIN");
            # Don't delete - just leave empty chain to avoid "too many links" error
        }
        if(&firewall_chain_exists("BLOCKLISTOUT") == 0) {
            run("$IPTABLES -F BLOCKLISTOUT");
            # Don't delete - just leave empty chain to avoid "too many links" error
        ...
        # Clean up blocklist-specific chains (these should be safe to delete)
        foreach my $blocklist (@blocklists) {
            if(&firewall_chain_exists("${blocklist}_DROP") == 0) {
                run("$IPTABLES -F ${blocklist}_DROP");
                # Try to delete but suppress error if it fails
